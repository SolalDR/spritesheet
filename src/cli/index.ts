...
let spritesheetConfig: CLISpritesheetConfig = {
  ...args,
  ...args.s,
  ...args.spritesheet,
}
...
spritesheetConfig.height =
  spritesheetConfig.height || spritesheetConfig.h || spritesheetConfig.width
spritesheetConfig = {
...
if (!outputConfig.name)
  console.log('No output name was provided, using "spritesheet" by default')
outputConfig.name = outputConfig.name || 'spritesheet'
if (!args.o?.dir && !args.output?.dir)
  console.log('No output dir was provided, using current directory by default')
...
const globsSet = args.files
  ? args.files instanceof Array || [String(args.files)]
  : []
